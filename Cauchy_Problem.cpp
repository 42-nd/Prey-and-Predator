...
// F - правая часть системы ДУ
    ...
    vector<double> Cauchy_Problem<double>::Adams_Predictor_Corrector_Scheme(
        Difference_Scheme_Type FD_Type,
        double Time_Begin,
        double Time_End,
        ...
        double h = (Time_End - Time_Begin);
        vector<double> Predicted_Solution(Variable_Number);
        vector<double> Corrected_Solution(Variable_Number);
        Point P(Time_Begin, 0, 0);

        for (int i = 0; i < Variable_Number; ++i) {
            Predicted_Solution[i] = Initial_Conditions[3][i] + h * (
                55.0 / 24.0 * F(Initial_Conditions[3], P)[i] -
                59.0 / 24.0 * F(Initial_Conditions[2], P)[i] +
                37.0 / 24.0 * F(Initial_Conditions[1], P)[i] -
                9.0 / 24.0 * F(Initial_Conditions[0], P)[i]
                );
        }
        ...
        vector<double> Previous_Solution = Predicted_Solution;
        Corrected_Solution = Predicted_Solution;

        for (int iter = 0; iter < max_iterations; ++iter) {
            Point P_next(Time_Begin + h, 0, 0);

            for (int i = 0; i < Variable_Number; ++i) {
                Corrected_Solution[i] = Initial_Conditions[3][i] + h / 24.0 * (
                    9.0 * F(Corrected_Solution, P_next)[i] +
                    19.0 * F(Initial_Conditions[3], P)[i] -
                    5.0 * F(Initial_Conditions[2], P)[i] +
                    F(Initial_Conditions[1], P)[i]
                    );
            }

            double error = 0.0;
            for (int i = 0; i < Variable_Number; ++i) {
                error += abs(Corrected_Solution[i] - Previous_Solution[i]);
            }
            if (error < tolerance) {
                break;
            }

            Previous_Solution = Corrected_Solution;
        }
